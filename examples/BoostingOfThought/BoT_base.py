"""
Implementation of the base model with the experience recall used by BoT.
"""
import re
import random
...
class ToEThoughtGenerator:
    """A thought generator with the trial-and-error (ToE) experience of BoT."""
    ...
    def generate_thoughts(
        ...
        This function contains an important observation:
         ...
         will always produce the similar responses. In most cases, 1, 1 will be selected to
         ...
         can recall experiences to avoid invalid reasoning steps before reasoning, it will
         1. generate multiple different reasoning steps, instead of trapping in the similar ones.
    ...
    def measure_similarity(self, thought_a, thought_b):
        ...
        similarity_answer = self.request_model.extract_response_contents(responses)[0]

        # Extract the similarity score
        score = 0
        match = re.search(
            r"Similarity score[^0-9]*(\d+(\.\d+)?)", similarity_answer, re.I
        )
        if match:
            score = float(match.group(1))
        return score

...
class PolicySystemPrompts(BaseSystemPrompts):
    ...
    policy_guided_generation_prompt: str = (
        """As an expert in problem-solving, you are skilled in methodical, step-by-step reasoning guided by policies, each presenting a general-purpose reasoning instruction for one step. Your response should only include one next step. Start by reviewing the problem and reasoning steps, along with their corresponding policies. Then, follow the given specific policy to directly generate the next step. Remember, your next step should include a precise analysis and the corresponding mathematical expression. This comprehensive approach will ensure a thorough solution. Utilize Python Programming as an auxiliary tool when necessary."""
    )
